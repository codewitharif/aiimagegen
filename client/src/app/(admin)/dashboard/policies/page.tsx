...
export default function PoliciesPage() {
  ...
  const getCategoryStyles = (category: string) => {
    switch (category) {
      case 'Health': return { color: 'text-emerald-600', bg: 'bg-emerald-500/10', badge: 'bg-emerald-50 text-emerald-600', icon: 'text-emerald-500' };
      case 'Life': return { color: 'text-emerald-600', bg: 'bg-emerald-500/10', badge: 'bg-emerald-50 text-emerald-600', icon: 'text-emerald-500' };
      case 'Motor': return { color: 'text-orange-600', bg: 'bg-orange-500/10', badge: 'bg-orange-50 text-orange-600', icon: 'text-orange-500' };
      case 'Travel': return { color: 'text-purple-600', bg: 'bg-purple-500/10', badge: 'bg-purple-50 text-purple-600', icon: 'text-purple-500' };
      default: return { color: 'text-slate-600', bg: 'bg-slate-500/10', badge: 'bg-slate-50 text-slate-600', icon: 'text-slate-500' };
    }
  };
                  ...
                  <div className="p-5">
                    ...
                    <div className="grid grid-cols-2 gap-3 mb-4">
                      <div className="bg-slate-50 p-2.5 rounded-xl border border-slate-100">
                        <p className="text-[9px] font-bold text-slate-400 uppercase tracking-tight mb-0.5">Premium</p>
                        <p className="text-xs font-bold text-slate-900">{policy.premium}</p>
                      </div>
                      <div className="bg-slate-50 p-2.5 rounded-xl border border-slate-100">
                        <p className="text-[9px] font-bold text-slate-400 uppercase tracking-tight mb-0.5">Coverage</p>
                        <p className="text-xs font-bold text-slate-900 truncate">{policy.coverage}</p>
                      </div>
                    </div>
                  ...
                  {POLICIES.map((policy, i) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-6 py-4 text-sm font-bold text-slate-900">{policy.premium}</td>
                        <td className="px-6 py-4 text-sm font-medium text-slate-600">{policy.coverage}</td>
